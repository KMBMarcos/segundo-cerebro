...
;
;
;         __    __  ______   ______  __    __    _____  ______  __      __      ______   ______
;        |  \  |  \/      \ /      \|  \  /  \  |     \/      \|  \    /  \    /      \ /      \    
;        | ▓▓  | ▓▓  ▓▓▓▓▓▓\  ▓▓▓▓▓▓\ ▓▓ /  ▓▓   \▓▓▓▓▓  ▓▓▓▓▓▓\\▓▓\  /  ▓▓   |  ▓▓▓▓▓▓\  ▓▓▓▓▓▓\
;        | ▓▓__| ▓▓ ▓▓  | ▓▓ ▓▓   \▓▓ ▓▓/  ▓▓      | ▓▓ ▓▓__| ▓▓ \▓▓\/  ▓▓    | ▓▓  | ▓▓ ▓▓___\▓▓
;        | ▓▓    ▓▓ ▓▓  | ▓▓ ▓▓     | ▓▓  ▓▓  __   | ▓▓ ▓▓    ▓▓  \▓▓  ▓▓     | ▓▓  | ▓▓\▓▓    \
...
Manual explicito de configuracion propuesta para Hockjay OS variante Parrot.
...
```
Si al momento de hacer el comando "make" sale este error:
cc -std=c99 -pedantic -Wall -Wextra -D_POSIX_C_SOURCE=200112L -DVERSION=\"0.6.3\"  -c -o sxhkd.o src/sxhkd.c
src/sxhkd.c:26:10: fatal error: xcb/xkb.h: No such file or directory
   26 | #include <xcb/xkb.h>
      |          ^~~~~~~~~~~
compilation terminated.
make: *** [<builtin>: sxhkd.o] Error 1

Es porque hacen falta dependencias para sxhkd, solo hay que ejecutar los siguientes comandos:
sudo apt update
sudo apt install libxcb-xkb-dev libxcb-util0-dev libxcb-keysyms1-dev libxcb-ewmh-dev libxcb-icccm4-dev libxcb-randr0-dev libxcb-xinerama0-dev libxcb-shape0-dev

Vuelves al directorio de sxhkd y ejecutas estos comandos:
make clean (para limpiar compiladores anteriores)
Y continuamos...
```
...
cd /home/<username>/Descargas/bspwm/
ls examples/bspwmrc (verificamos si existe)
...
# ------------------------------------------------------------------------------------------------
# El archivo indicado estara en: ~/.config/sxhkd/sxhkdrc

### 4.5 Instalamos kitty terminal
...
```
What is Ninja?
Ninja is a small build system with a focus on speed. It is designed to be used with a higher-level build generator, 
such as CMake or Meson. Ninja reads a simple text-based build file (usually named build.ninja) that describes the 
build rules and dependencies, and then it efficiently executes the build commands.

En pocas palabras, es un software que nos ayudara a automatizar cosas.
```
ninja -C build
sudo ninja -C build install

### Actualizamos Firefox y lo enjaulamos(firejail)
Descargar el archivo tar de la pagina oficial: https://www.firefox.com/en-US/browsers/desktop/linux/
sudo su
cd /
chown <username>:<username> opt/
cd !$

- Ya situados en el directorio /opt/
su <username> (para salir del usuario root)
mv /home/<username>/Downloads/<archivo tar descargado> .
tar -xf <archivo tar desacargado> (para descomprimir)
rm <archivo tar descargado> (para borrar el comprimido)
cd firefox/
./firefox (Probamos que si abre)
sudo su
apt install firefail
exit (para salir de la sesion como root)
firejail ./firefox (para probar si se lanza el firefox)
...
### Fonts descargadas de Nerd Fonts:
- Terminal: Hack
- Polybar: Iconsolata
...
mv /home/<username>/Downloads/InconsolataLGC.zip
unzip Hack.zip
rm Hack.zip
unzip InconsolataLGC.zip
rm InconsolataLGC.zip

### (Opcional)11. Instalamos el addon 'FoxyProxy' para Firefox.
...
feh --bg-fill /home/<username>/Pictures/IMAGENES/MysticHorizon.jpg
super + alt + q (para cerrar sesion)
...
En el archivo ~/.config/polybar/current.ini, para configurar la estructura de la polybar
Quitamos el modulo de volumen, comentando esa linea.

Estructura de un nuevo módulo:
 
Previamente tenemos que crear una carpeta en "~/.config/bin"

Creamos este modulo en el archivo current.ini. Este modulo lo que hace es mostrarnos la ip asignada.
[module/tumodulo]
type = custom/script
interval = 2
